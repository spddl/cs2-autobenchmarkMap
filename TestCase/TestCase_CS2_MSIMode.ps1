...
# GoInterruptPolicy is necessary for this https://github.com/spddl/GoInterruptPolicy/releases/tag/v1.3.0
$devobj = '\Device\NTPNP_PCI0021'

$TestCases = @()
for ($i = 0; $i -lt 3; $i++) {
 # number of runs
	$TestCases += @{ Value = '0'; Name = 'MSI Off' } # The value that will be changed
	$TestCases += @{ Value = '1'; Name = 'MSI On' }
}
...
if ($id -ne $TestCases.Count) {
	...
	Start-Process -FilePath GoInterruptPolicy -ArgumentList "-devobj $devobj -msisupported $($TestCases[$id].Value) -restart" -Wait -NoNewWindow
	Write-Host 'Sleep 5s'
	Start-Sleep -Seconds 5
