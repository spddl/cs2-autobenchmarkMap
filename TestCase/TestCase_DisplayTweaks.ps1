...
for ($i = 0; $i -lt 5; $i++) {
 # number of runs
	$TestCases += @{ Value = 1; Name = 'Tweaks_On' } # The value that will be changed
	$TestCases += @{ Value = 0; Name = 'Tweaks_Off' }
}
...
if ($id -ne $TestCases.Count) {
	...
	if ($TestCases[$id].Value -eq 1) {
		Write-Host 'DisplayTweaks.reg'
		Start-Process -FilePath 'reg.exe' -ArgumentList "import `"$PSScriptRoot\DisplayTweaks.reg`"" -Wait -NoNewWindow
	} else {
		Write-Host 'DisplayTweaks - Default.reg'
		Start-Process -FilePath 'reg.exe' -ArgumentList "import `"$PSScriptRoot\DisplayTweaks_Default`"" -Wait -NoNewWindow
	}
